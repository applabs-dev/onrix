...
import {
  LemonSqueezyProductDialog,
  type LemonSqueezyProductData,
} from './lemonsqueezy-product-dialog'

type LemonSqueezyProductsVisualEditorProps = {
  value: string
  onChange: (value: string) => void
}

export function LemonSqueezyProductsVisualEditor({
  value,
  onChange,
}: LemonSqueezyProductsVisualEditorProps) {
  const { t } = useTranslation()
  const [searchText, setSearchText] = useState('')
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editData, setEditData] = useState<LemonSqueezyProductData | null>(null)

  const products = useMemo(() => {
    const parsed = safeJsonParseWithValidation<unknown[]>(value, {
      fallback: [],
      validator: isArray,
      validatorMessage: t('Lemon Squeezy products must be a JSON array'),
      context: 'lemonsqueezy products',
    })

    return parsed.filter(
      (item): item is LemonSqueezyProductData =>
        typeof item === 'object' &&
        item !== null &&
        'name' in item &&
        'productId' in item &&
  ...
  const handleSave = (data: LemonSqueezyProductData) => {
    const parsed = safeJsonParseWithValidation<unknown[]>(value, {
      fallback: [],
      validator: isArray,
      silent: true,
  ...
  const handleDelete = (product: LemonSqueezyProductData) => {
    const parsed = safeJsonParseWithValidation<unknown[]>(value, {
      fallback: [],
      validator: isArray,
      silent: true,
  ...
  const handleEdit = (product: LemonSqueezyProductData) => {
    setEditData(product)
    setDialogOpen(true)
  }
